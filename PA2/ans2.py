...
def bSpline(t, p0, p1, p2, p3):
    return (1 / 6) * ((-t ** 3 + 3 * t ** 2 - 3 * t + 1) * p0 + (3 * t ** 3 - 6 * t ** 2 + 4) * p1 + (
                -3 * t ** 3 + 3 * t ** 2 + 3 * t + 1) * p2 + t ** 3 * p3)


def cowDirection(t, p0, p1, p2, p3):
    direction = (1 / 6) * ((-3 * t ** 2 + 6 * t - 3) * p0 + (9 * t ** 2 - 12 * t) * p1 + (
                -9 * t ** 2 + 6 * t + 3) * p2 + 3 * t ** 2 * p3)
    ...
    R_pitch = np.array([[np.cos(yaw), 0., np.sin(yaw)],
                   [0., 1., 0.],
...
def display():
    ...
    if count < 6:
        ...
        for i in range(count):
            drawCow(positions[i], cursorOnCowBoundingBox)
    elif count == 6:
        if not initialTime:
            animStartTime = glfw.get_time()
            initialTime = True
            rollingPosition = positions[0].copy()
        animTime = glfw.get_time() - animStartTime
        ...
        if animTime < 18.0 and t >= 0 and t < 6.0:
            for i in range(6):
                if i <= t and t < i + 1:
                    t %= 1
                    spline = bSpline(t, positions[(5 + i) % 6], positions[i % 6], positions[(i + 1) % 6],
                                     positions[(i + 2) % 6])
                    cowDirection(t, positions[(5 + i) % 6], positions[i % 6], positions[(i + 1) % 6],
                                 positions[(i + 2) % 6])
                    break
        else:
            count = 0
        ...
        setTranslation(rollingPosition, getTranslation(spline))
        drawCow(rollingPosition, False)  # Draw cow.
    if count != 6:
        drawCow(cow2wld, cursorOnCowBoundingBox)
    glFlush()
...
def onMouseButton(window, button, state, mods):
    global isDrag, V_DRAG, H_DRAG, count, positions, cow2wld, isNotInitialClick
    GLFW_DOWN = 1;
    GLFW_UP = 0;
    ...
    if isDrag:
        ...
        if isDrag == V_DRAG:
            ...
            if cursorOnCowBoundingBox:
                ...
                print(getTranslation(cow2wld), currentPos)
